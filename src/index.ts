...
const castToHub = async ({ color, name, address }: ColorMintPayload) => {
  ...
  const tweetText =
    text +
    `\n\nminted by ${
      ensName ? ensName : shortenAddressFirstFourLastThree(address)
    }`;

  text += `\n\nminted by ${
    fid ? " " : ensName ? ensName : shortenAddressFirstFourLastThree(address)
  }`;

  const imageUrl = `https://www.palettes.fun/api/basecolors/image/${color
    .replace("#", "")
    .toLowerCase()}.png`;
  ...
  try {
    setTimeout(async () => {
      ...
      const parsedTokenData = JSON.parse(
        Buffer.from(
          tokenData.replace("data:application/json;base64,", ""),
          "base64"
        ).toString("utf-8")
      );
      ...
      const name = parsedTokenData.attributes.find(
        (attribute: any) => attribute.trait_type === "Color Name"
      )?.value;
...
async function createSubscription(address: string) {
  ...
  tokenMinted = await publicClient.transport.subscribe({
...
const interval = setInterval(() => {
  if (Date.now() - lastProcessedAt > 60_000 * 5 || !tokenMinted) {
    try {
      tokenMinted.unsubscribe();
    } catch (error) {
      console.error("Error unsubscribing the wss");
    }
    createSubscription(CONTRACT_ADDRESS);
  }
}, 60_000 * 5);
